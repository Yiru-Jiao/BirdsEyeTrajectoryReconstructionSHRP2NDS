...
### An important note before we start 开始之前的重要提示
As aforementioned, the authors from TU Delft returned the derived data to VTTI which is the owner of SHRP2 NDS data. We as the authors did not (and do not have the right to) employ any restrictions on using SHRP2 NDS data. However, very unfortunately and we believe no one would like to see it happens, many Chinese researchers' applications for the dataset have been rejected due to "export control licensing concerns" raised by Virginia Tech Office of Export and Secure Research Compliance. In response to this, we suggest actions below:\
如前所述，我们在TU Delft的团队将衍生数据集返回了VTTI、即SHRP2 NDS数据的拥有者。我们没有（也无权）对使用SHRP2 NDS施加限制。然而非常遗憾地且我们相信没有人愿意看到地，不少中国机构研究者的申请被Virginia Tech出口与安全研究合规办公室以“涉及出口管制许可的关切”为由拒绝了。应对这一情况，我们建议如下行动：
- Regardless of nationality, you are invited to vote on a [poll about your application experience](https://github.com/Yiru-Jiao/BirdsEyeTrajectoryReconstructionSHRP2NDS/discussions/4). This is to better know about the procedure and provide evidence for helping everyone in the community get fair use of the data.\
  无论国籍，我们邀请您参与讨论区关于[申请经验的投票](https://github.com/Yiru-Jiao/BirdsEyeTrajectoryReconstructionSHRP2NDS/discussions/4)，以更好地了解正在发生什么、并作为证据帮助每个人争取公平使用数据的权利。
- The only alternative dataset, as far as we know, is [DRAMA](https://usa.honda-ri.com/datasets) from Honda. Please feel free to apply for this dataset and reuse our [methods](https://github.com/Yiru-Jiao/BirdsEyeTrajectoryReconstructionSHRP2NDS/blob/main/pdfs/MethodExplanation.pdf) to reconstruct BEV trajectories. Useful code repositories can be referred to [Reconstruct100CarNDSData](https://github.com/Yiru-Jiao/Reconstruct100CarNDSData) and [GSSM](https://github.com/Yiru-Jiao/GSSM/tree/main/src_trajectory_reconstruction).\
  目前唯一的替代选择，据我们所知，是Honda的[DRAMA](https://usa.honda-ri.com/datasets)数据集。请按您所需考虑申请该数据集、重用我们的[方法](https://github.com/Yiru-Jiao/BirdsEyeTrajectoryReconstructionSHRP2NDS/blob/main/pdfs/MethodExplanation.pdf)重建鸟瞰轨迹。可以参考的代码仓包括[Reconstruct100CarNDSData](https://github.com/Yiru-Jiao/Reconstruct100CarNDSData)以及[GSSM](https://github.com/Yiru-Jiao/GSSM/tree/main/src_trajectory_reconstruction)。
- If your application was rejected due to "export control licensing concerns" while you are still seeking a chance, please feel free to contact me to reuse the license used for creating this dataset. Our license will be expired by the end of June 2026, thus a study using it should be finished/submitted/made public before that date. As a consequence, I am required to be involved in your research as data provider/curator.\
  如果您的申请已经被以“涉及出口管制许可的关切”理由拒绝，而且仍寻求使用本数据集，请按您所需考虑联系我以重用用于创建本数据集的许可证。我们的许可证将于2026年6月底到期，因此能够以此证进行的研究需要届时完成/投出/公开。这样做的影响是，我将需要作为数据提供者/构建者被纳入您所进行的研究。

As a Chinese, I feel upset, and I sincerely appologise for not considering potential political influence which is leading to the inconvenience.\
对于没能考虑到造成如今不便的政治影响，我作为中国人感到不忿，向我国的同僚们致歉。
